...
func NewOpenCodeScanner(cfg config.Config) *OpenCodeScanner {
	bin := cfg.BinFor(types.PlatformOpenCode)
...
func getOpenCodeDBPath(bin string) string {
	...
	if err != nil {
		// Only warn when the command exists but fails (silent when not found)
...
func (s *OpenCodeScanner) Platform() types.Platform { return types.PlatformOpenCode }
...
func (s *OpenCodeScanner) ScanProjects() ([]types.Project, error) {
	if s.dbPath == "" {
		return nil, nil
	}
	...
	for rows.Next() {
		...
		sess := types.Session{
			ID:          id,
			Platform:    types.PlatformOpenCode,
	...
	for _, wt := range worktreeOrder {
		projects = append(projects, types.Project{
			Name:     types.ProjectShortName(wt),
			FullPath: wt,
...
func (s *OpenCodeScanner) DeleteSession(sess types.Session) error {
...
func (s *OpenCodeScanner) DeleteProject(p types.Project) error {
...
func (s *OpenCodeScanner) ResumeCmd(sess types.Session) []string {
